...
const { verifyIsLoggedIn, verifyIsAdmin } = require('../authentication/authMiddlewares')
...
router.get('/', async (req, res) => {
    ...
    if (!allProducts) return res.send("No products found")
...
router.delete('/:id', verifyIsLoggedIn, verifyIsAdmin, async (req, res) => {
    ...
    if (!productToDelete) return res.status(404).send("No product found")
